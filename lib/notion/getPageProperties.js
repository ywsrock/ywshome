import { getTextContent, getDateValue, defaultMapImageUrl, getBlockValue } from 'notion-utils'
...
async function getPageProperties(id, block, schema, authToken) {
  ...
  const rawProperties = Object.entries(getBlockValue(block?.[id])?.properties || [])
        ...
        case 'person': {
          ...
          for (let i = 0; i < rawUsers.length; i++) {
            if (rawUsers[i][0][1]) {
              ...
              const res = await api.getUsers(userId, NOTION_GOT_OPTIONS)
              const resValue =
